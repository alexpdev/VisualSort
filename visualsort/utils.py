...
from visualsort.conf import configuration
...
def get_screen():
    conf = configuration()
    screen = Screen()
    screen.setup(*conf["screensize"])
    screen.bgcolor(conf["background"])
    screen.tracer(conf.get("tracer"))
    screen.delay(conf.get("delay"))
    screen.speed = conf.get("speed")
    ...
    screen.blockwidth = conf["blockwidth"]
    screen.increment = screen.blockheight = conf["blockheight"]
